/****************************************************************************************************************************
  AsyncFSEditor_Teensy41.cpp - Dead simple AsyncFSWebServer for Teensy41 QNEthernet
...
static void loadExcludeList(FS* fs, const char *filename)
{
  ...
  if (fs)
  {
    Serial.print("loadExcludeList: FS * = 0x"); Serial.println((uint32_t) fs, HEX);
  }
  else
  {
    Serial.println("AsyncFSEditor::loadExcludeList: Error FS NULL");
  }
...
static bool isExcluded(FS* fs, const char *filename)
{
  if (fs)
  {
    Serial.print("isExcluded: FS * = 0x"); Serial.println((uint32_t) fs, HEX);
  }
  else
  {
    Serial.println("AsyncFSEditor::isExcluded: Error FS NULL");
  }
...
AsyncFSEditor::AsyncFSEditor(/*const*/ FS* fs, const String& username, const String& password)
:_fs(fs)
,_username(username)
,_password(password)
...
{ 
  ...
  if (fs)
  {
    Serial.print("AsyncFSEditor: FS * = 0x"); Serial.println((uint32_t) fs, HEX);
  }
  else
  {
    Serial.println("AsyncFSEditor: Error FS NULL");
  }
#endif  
}
...
void AsyncFSEditor::handleRequest(AsyncWebServerRequest *request)
{
  ...
  if(request->method() == HTTP_GET)
  {
    ...
    else 
    {
      ...
      {
        Serial.println("AsyncFSEditor::handleRequest: Sending AsyncWebServerResponse");
